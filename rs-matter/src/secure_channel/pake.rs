...
impl Pake {
    ...
    async fn handle_pasepake1(
        ...
        {
            let pase = rx.exchange().matter().pase_mgr.borrow();
    ...
    async fn handle_pbkdfparamrequest(
        ...
        let resp = {
            ...
            (rx.exchange().matter().pase_mgr.borrow().rand)(&mut our_random);
    ...
    async fn check_session(&mut self, exchange: &mut Exchange<'_>) -> Result<bool, Error> {
        let mut rx = exchange.rx().await?; // Wait for the next packet to arrive

        if rx.exchange().matter().pase_mgr.borrow().session.is_none() {
            rx.consume();
            drop(rx);
