...
impl<'a, 'b> ReportDataStreamingResp<'a, 'b> {
    ...
    pub fn start(&mut self, req: &ReportDataReq) -> Result<(), Error> {
        self.0.reset();
    ...
    pub fn finish_chunk(&mut self, req: &ReportDataReq) -> Result<&[u8], Error> {
        self.complete(req, true)
    }

    pub fn finish(&mut self, req: &ReportDataReq) -> Result<&[u8], Error> {
        self.complete(req, false)
    }

    fn complete(&mut self, req: &ReportDataReq, more_chunks: bool) -> Result<&[u8], Error> {
