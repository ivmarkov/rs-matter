...
use domain::{
    base::{
        ...
        message_builder::{AdditionalBuilder, AnswerBuilder, PushError},
        name::FromStrError,
        wire::{Composer, ParseError},
        Message, MessageBuilder, Name, RecordSectionBuilder, Rtype, ToName,
    },
    dep::octseq::Truncate,
};
...
impl Services for () {
    fn for_each<F>(&self, _callback: F) -> Result<(), Error>
    where
        F: FnMut(&Service) -> Result<(), Error>,
    {
        Ok(())
...
mod tests {
    ...
    use domain::base::{
        header::Flags,
        iana::{Class, Opcode, Rcode},
        Message, MessageBuilder, Name, RecordSection, Rtype, ToName,
    };
            ...
            while answers.peek().is_some() && expectations.peek().is_some() {
                ...
                match (answer.data(), &expected.details) {
                    ...
                    (AllRecordData::Txt(txt), AnswerDetails::Txt(kvs)) => {
                        ...
                        if let Some(t) = txt.next() {
                            panic!("Unexpected TXT string {:?}", t);
                        }

                        if let Some((k, v)) = kvs.next() {
                            panic!("Missing TXT string {k}={v}");
                        }
                    }
