...
impl<'a> MdnsService<'a> {
        ...
        S: UdpSend,
    {
        loop {
            ...
            for remote_addr in core::iter::once(SocketAddr::V4(SocketAddrV4::new(
            ...
            .chain(
                ...
                if len > 0 {
                    ...
                    send.send_to(remote_addr, &send_buf[..len]).await?;
                }
            }
        }
    }
    ...
    {
        loop {
            let (len, remote) = recv.recv_from(recv_buf).await?;
            ...
            if len > 0 {
                ...
                send.send_to(remote, &send_buf[..len]).await?;
            }
        }
    }
}
