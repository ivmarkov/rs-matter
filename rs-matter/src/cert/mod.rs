...
impl<'a> CertRef<'a> {
    ...
    fn get_subject_key_id(&self) -> Result<&[u8], Error> {
        try_find_map(self.extensions()?.iter(), |extension| {
            Ok(if let Extension::SubjectKeyId(id) = extension {
                Some(id.0)
            } else {
                None
            })
        })?
        .ok_or(ErrorCode::Invalid.into())
    }

    fn is_authority(&self, their: &CertRef) -> Result<bool, Error> {
        let their_subject = their.get_subject_key_id()?;

        let authority = try_find_map(self.extensions()?.iter(), |extension| {
...
impl<'a> CertVerifier<'a> {
    ...
    pub fn add_cert(self, parent: &'a CertRef<'a>, buf: &mut [u8]) -> Result<Self, Error> {
        ...
        k.verify_msg(asn1.iter().copied(), self.cert.signature()?)
            .map_err(|e| {
                error!(
...
pub fn try_find_map<T, E, P, I, R>(iter: I, map: P) -> Result<Option<R>, E>
where
    I: Iterator<Item = Result<T, E>>,
    P: Fn(T) -> Result<Option<R>, E>,
{
    for val in iter {
        let val = val?;
