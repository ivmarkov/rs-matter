...
use crate::{
    data_model::cluster_basic_information::BasicInfoConfig, error::Error,
    secure_channel::spake2p::VerifierOption, transport::network::BtAddr, CommissioningData,
};
...
impl AdvData {
    ...
    pub const fn new(dev_det: &BasicInfoConfig, comm_data: &CommissioningData) -> Self {
        Self {
            ...
            pin: match comm_data.verifier.data {
                VerifierOption::Password(password) => password,
                _ => 0,
            },
        }
    }

    /// The PIN to be used when pairing.
