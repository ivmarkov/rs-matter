...
impl BluerGattPeripheral {
        ...
        F: Fn(GattPeripheralEvent) + Send + Sync + 'static,
    {
        ...
        // Need to register our own agent or else the Matter comissioner will not be able to pair with us
        session
            .register_agent(Agent {
                //request_passkey: Some(Box::new(move |_| Box::pin(async move { Ok(pin) }))),
                ..Default::default()
            })
            .await?;
