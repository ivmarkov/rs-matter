...
impl Case {
    ...
    async fn handle_casesigma3(
        ...
        let status = {
            let fabric_mgr = rx.exchange().matter().fabric_mgr.borrow();
            ...
            if let Some(fabric) = fabric {
                ...
                if let Err(e) = Case::validate_certs(fabric, &initiator_noc, initiator_icac_mut) {
                    ...
                    let peer_addr = rx
                        .exchange()
                        .with_session(|sess| Ok(sess.get_peer_addr()))?;
    ...
    async fn handle_casesigma1(
        ...
        let key_pair = KeyPair::new(rx.exchange().matter().rand)?;
        ...
        (rx.exchange().matter().rand)(&mut our_random);
